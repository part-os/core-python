...
class AccountMapper(BaseMapper):
    ...
    def map(cls, resource):
        ...
        mapped_result['billing_addresses'] = map(BillingAddressMapper.map, resource['billing_addresses'])
...
class BillingAddressMapper(BaseMapper):
    ...
    def map(cls, resource):
        mapped_result = {}
        field_keys = ['address1', 'address2', 'city', 'country', 'id', 'postal_code', 'state']
        for key in field_keys:
            mapped_result[key] = resource.get(key, None)
        return mapped_result
...
class ContactMapper(BaseMapper):
    ...
    def map(cls, resource):
        mapped_result = {}
        field_keys = ['account_id', 'created','email', 'first_name', 'id', 'last_name', 'notes', 'phone', 'phone_ext']
        for key in field_keys:
            mapped_result[key] = resource.get(key, None)
